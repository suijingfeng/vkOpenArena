...
/*
===============
VM_ValueToSymbol: Assumes a program counter value
===============
*/
...
/*
============
VM_DllSyscall: Dlls will call this directly
 ...
 rcg010206 The horror; the horror.
  ...
  The syscall mechanism relies on stack manipulation to get its args.
   ...
   On PowerPC Linux, these parameters are not necessarily passed on the stack,
   so while (&arg[0] == arg) is true, (&arg[1] == 2nd function parameter) is not necessarily accurate, 
   as arg's value might have been stored to the stack or other piece of scratch memory to give it a valid address, 
   but the next parameter might still be sitting in a register.

  Quake's syscall system also assumes that the stack grows downward,
   and that any needed types can be squeezed, safely, into a signed int.

  This hack below copies all needed values for an argument to a array in memory, so that Quake can get the correct values. 
  This can also be used on systems where the stack grows upwards, as the presumably standard and safe stdargs.h macros are used.

  As for having enough space in a signed int for your datatypes, well,
   it might be better to wait for DOOM 3 before you start porting.  :)

  The original code, while probably still inherently dangerous, seems to work well enough for the platforms it already works on.
  Rather than add the performance hit for those platforms, the original code is still in use there.
...
vmHeader_t *VM_LoadQVM( vm_t *vm, qboolean alloc, qboolean unpure)
{
	...
	if(header.h->vmMagic == VM_MAGIC_VER2)
	{
		...
		memcpy(vm->jumpTableTargets, (unsigned char *) header.h + header.h->dataOffset +
...
vm_t* VM_Create(const char *module, intptr_t (*systemCalls)(intptr_t *), vmInterpret_t interpret )
{
    ...
    strncpy( vm->name, module, sizeof(vm->name)-1 );
...
intptr_t QDECL VM_Call(struct vm_s *vm, int callnum, ... )
{
    ...
    struct vm_s* oldVM = currentVM;
	currentVM = vm;
	lastVM = vm;

//	if ( vm_debugLevel )
//	  Com_Printf(S_COLOR_YELLOW "VM_Call( %d )\n", callnum );
...
void VM_VmProfile_f( void )
{
	...
	vmSymbol_t** sorted = Z_Malloc( vm->numSymbols * sizeof( *sorted ) );
	sorted[0] = vm->symbols;
...
/*
===============
VM_LogSyscalls: Insert calls to this while debugging the vm compiler
===============
*/
...
/*
=================
VM_BlockCopy: Executes a block copy operation within currentVM data space
=================
*/
void VM_BlockCopy(unsigned int dest, unsigned int src, size_t n)
{
	...
	|| ((dest + n) & dataMask) != dest + n 	|| ((src + n) & dataMask) != src + n)
	{
