...
static int R_ComputeFogNum( mdvModel_t *model, trRefEntity_t *ent )
{
	...
	// FIXME: non-normalized axis issues
	mdvFrame_t* mdvFrame = model->frames + ent->e.frame;
...
static int R_ComputeLOD( trRefEntity_t *ent )
{
	...
	else
	{
		...
		else
		{
			...
			mdvFrame_t* frame = tr.currentModel->mdv[0]->frames;
...
void R_MDRAddAnimSurfaces( trRefEntity_t *ent )
{
	...
	qboolean personalModel = (ent->e.renderfx & RF_THIRD_PERSON) && 
...
void R_AddMD3Surfaces( trRefEntity_t *ent )
{
	...
	mdvModel_t* model = tr.currentModel->mdv[lod];
