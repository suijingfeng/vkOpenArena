	...
	qboolean useLightVector, qboolean useLightVertex, qboolean parallax, qboolean tcgen)
{
	...
	if (r_normalMapping->integer)
	{
		...
		else if ((lightmap || useLightVector || useLightVertex) && (diffuseImg = diffuse->bundle[TB_DIFFUSEMAP].image[0]))
		{
			...
			R_StripExtension(diffuseImg->imgName, normalName, MAX_QPATH);
	...
	if (r_specularMapping->integer)
	{
		...
		else if ((lightmap || useLightVector || useLightVertex) && (diffuseImg = diffuse->bundle[TB_DIFFUSEMAP].image[0]))
		{
			...
			R_StripExtension(diffuseImg->imgName, specularName, MAX_QPATH);
...
static void ScanAndLoadShaderFiles( void )
{
    ...
    {
        ri.Printf( PRINT_ALL, "****************************************\n");
        ri.Printf( PRINT_ALL, "Total %d Shader Files need to be loaded.\n", numShaderFiles);
        ri.Printf( PRINT_ALL, "****************************************\n");
    }
		...
		while(1)
		{
			...
			if(token[0] != '{' || token[1] != '\0')
			{
				ri.Printf(PRINT_WARNING, "WARNING: Ignoring shader file %s. Shader \"%s\" on line %d missing opening brace",
							filename, shaderName, R_GetCurrentParseLine());
				if (token[0])
				{
	...
	char* hashMem = ri.Hunk_Alloc( size * sizeof(char *), h_low );
