...
static void CON_Show(void)
{
	if( ttycon_on )
	{
		...
		if(ttycon_hide == 0)
		{
			size_t UNUSED_VAR size = write(STDOUT_FILENO, TTY_CONSOLE_PROMPT, strlen(TTY_CONSOLE_PROMPT));
			if (TTY_con.cursor)
			{
				for (i=0; i<TTY_con.cursor; i++)
				{
...
/*
==================
CON_Init: Initialize the console input (tty mode if possible)
==================
*/
void CON_Init( void )
{
